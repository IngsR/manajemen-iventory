...
import { adjustStockAction } from '@/actions/AdjustStockAction';
...
export default function AdjustmentPage() {
    ...
    const deltaColor = isNaN(deltaNum) || deltaNum === 0 ? 'text-gray-500' : deltaNum > 0 ? 'text-green-600' : 'text-red-600';
    ...
    return (
        <main className="max-w-2xl mx-auto p-6">
            <div className="flex items-center gap-3 mb-6">
                ...
                <h1 className="text-xl font-bold">Adjustment Stock (Koreksi Stok)</h1>
            </div>

            <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded text-sm text-amber-800">
                ⚠️ Adjustment langsung mengubah stok. Wajib disertai alasan yang jelas untuk keperluan audit.
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white border rounded-lg p-6 space-y-4">
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                        ...
                        <span className={`font-bold ${deltaColor}`}>
                            {delta && !isNaN(deltaNum) && deltaNum !== 0 ? (deltaNum > 0 ? `+${deltaNum}` : deltaNum) : ''}
                        </span>
                    </label>
                    <input
                        name="quantityDelta"
                        ...
                        placeholder="Positif = tambah, Negatif = kurangi (contoh: -5 atau +10)"
                        value={delta}
                        onChange={e => setDelta(e.target.value)}
                    />
                    <p className="text-xs text-gray-400 mt-1">Gunakan nilai negatif untuk mengurangi stok. Nol tidak diperbolehkan.</p>
                </div>

                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Alasan Koreksi *</label>
                    <input
                        name="reason"
                        type="text"
                        required
                        className="w-full border rounded px-3 py-2 text-sm border-amber-300"
                        placeholder="Contoh: Stock opname, Barang rusak, Koreksi input"
                    />
                </div>

                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Nomor Referensi</label>
                    <input name="referenceNumber" type="text" className="w-full border rounded px-3 py-2 text-sm" placeholder="ADJ-001, dll" />
                </div>
                ...
                >
                    {submitting ? 'Memproses...' : 'Proses Adjustment'}
                </button>
            </form>
                ...
                <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <div className="mt-2 grid grid-cols-3 gap-2 text-center text-sm">
                        ...
                        <div className="bg-white border rounded p-2">
                            ...
                            <p className={`text-lg font-bold ${result.stockAfter >= result.stockBefore ? 'text-green-600' : 'text-red-600'}`}>
                                {result.stockAfter - result.stockBefore > 0 ? '+' : ''}{result.stockAfter - result.stockBefore}
                            </p>
                        </div>
                        <div className="bg-white border rounded p-2">
                            <p className="text-gray-500">Stok Sekarang</p>
                            <p className="text-lg font-bold text-amber-700">{result.stockAfter}</p>
                        </div>
                    </div>
